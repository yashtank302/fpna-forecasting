...
# Features: time index + 1-month lags (use past info only)
...
# Drop first rows per dept (lags are NaN there)
...
test["Pred_Revenue"] = rf.predict(X_test)
...
# ---- Save outputs ----
out.to_csv(OUT_DIR / "rf_predictions.csv", index=False)
...
plt.plot(ops["Month"], ops["Pred_Revenue"], label="Random Forest Predicted")
plt.title("Operations: Actual vs Predicted — Random Forest")
plt.xlabel("Month"); plt.ylabel("Revenue"); plt.legend()
plt.tight_layout(); plt.savefig(OUT_DIR / "rf_ops_plot.png"); plt.close()

print("Done. Files saved in outputs/:")
print(" - rf_predictions.csv")
print(" - rf_metrics.csv")
print(" - rf_ops_plot.png")
